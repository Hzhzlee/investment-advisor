...
export const ProjectionChart: React.FC<ProjectionChartProps> = ({
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-white/95 p-3 text-xs shadow-lg backdrop-blur-md min-w-[240px]">
        <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-200">
          <span className="font-semibold text-slate-800">
            Year: <span className="font-mono text-blue-600">{label}</span>
          </span>
        ...
        <div className="space-y-1.5 font-mono">
          {dataItem.historical !== null && dataItem.historical !== undefined && (
            <div className="flex flex-col gap-0.5 pb-1 border-b border-slate-200">
              ...
              {dataItem.mcP10 !== undefined && (
                <div className="mt-2 pt-2 border-t border-slate-200 text-[11px] text-slate-500 space-y-0.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-emerald-700 font-semibold">{sym}{dataItem.mcP90.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Monte Carlo P50 (Median):</span>
                    <span className="text-sky-700 font-semibold">{sym}{dataItem.mcP50.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Monte Carlo P10:</span>
                    <span className="text-rose-700 font-semibold">{sym}{dataItem.mcP10.toLocaleString()}</span>
                  </div>
                </div>
              )}
            </>
  ...
  return (
    <section id="projections" className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 20, right: 25, left: 15, bottom: 25 }}>
              ...
              <Legend
                ...
                formatter={(val) => <span className="text-slate-700 font-medium mr-3">{val}</span>}
              />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between text-[11px] text-slate-500 border-t border-slate-200 pt-3">
